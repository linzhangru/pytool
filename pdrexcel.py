...
app.layout = html.Div([
    html.Div([
        ...
        dcc.Checklist(
            ...
            options = [
                {'label':"OOM_ADJ.Native"      ,'value':"native"     },
                {'label':"OOM_ADJ.Persist"     ,'value':"persist"    },
                {'label':"OOM_ADJ.SystemServer",'value':"system"     },
                {'label':"OOM_ADJ.Foreground"  ,'value':"foreground" },
                {'label':"OOM_ADJ.Visible"     ,'value':"visible"    },
                {'label':"OOM_ADJ.Perceptible" ,'value':"perceptible"},
                {'label':"OOM_ADJ.AServices"   ,'value':"aservices"   },
                {'label':"OOM_ADJ.Home"        ,'value':"home"       },
                {'label':"OOM_ADJ.BServices"   ,'value':"bservices"   },
                {'label':"OOM_ADJ.Cached"      ,'value':"cached"     }
            ],
...
def update_figure_oomadj(selected_items):
    ...
    for i in selected_items:
        if cmp(str(i),"native") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.Native"]),1), 
                                     y = df["OOM_ADJ.Native"].values,
                                     mode='lines+markers',
                                     name='Native'))
            print type(df["OOM_ADJ.Native"])

        if cmp(str(i),"persist") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.Persist"]),1), 
                                     y = df["OOM_ADJ.Persist"].values,
                                     mode='lines+markers',
                                     name='Persist'))
            print type(df["OOM_ADJ.Persist"])

        if cmp(str(i),"system") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.SystemServer"]),1), 
                                     y = df["OOM_ADJ.SystemServer"].values,
                                     mode='lines+markers',
                                     name='System'))
            print type(df["OOM_ADJ.SystemServer"])

        if cmp(str(i),"foreground") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.Foreground"]),1), 
                                     y = df["OOM_ADJ.Foreground"].values,
                                     mode='lines+markers',
                                     name='Foreground'))
            print type(df["OOM_ADJ.Foreground"])
            
        if cmp(str(i),"visible") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.Visible"]),1), 
                                     y = df["OOM_ADJ.Visible"].values,
                                     mode='lines+markers',
                                     name='Visible'))
            print type(df["OOM_ADJ.Visible"])

        if cmp(str(i),"perceptible") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.Perceptible"]),1), 
                                     y = df["OOM_ADJ.Perceptible"].values,
                                     mode='lines+markers',
                                     name='Perceptible'))
            print type(df["OOM_ADJ.Perceptible"])

        if cmp(str(i),"aservices") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.AServices"]),1), 
                                     y = df["OOM_ADJ.AServices"].values,
                                     mode='lines+markers',
                                     name='AServices'))
            print type(df["OOM_ADJ.AServices"])

        if cmp(str(i),"home") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.Home"]),1), 
                                     y = df["OOM_ADJ.Home"].values,
                                     mode='lines+markers',
                                     name='Home'))
            print type(df["OOM_ADJ.Home"])

        if cmp(str(i),"bservices") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.BServices"]),1), 
                                     y = df["OOM_ADJ.BServices"].values,
                                     mode='lines+markers',
                                     name='BServices'))
            print type(df["OOM_ADJ.BServices"])

        if cmp(str(i),"cached") == 0:
            traces.append(go.Scatter(x = np.arange(0,len(df["OOM_ADJ.Cached"]),1), 
                                     y = df["OOM_ADJ.Cached"].values,
                                     mode='lines+markers',
                                     name='Cached'))
            print type(df["OOM_ADJ.Cached"])
...
def update_figure_category(selected_items):
    print selected_items
    print len(selected_items)
